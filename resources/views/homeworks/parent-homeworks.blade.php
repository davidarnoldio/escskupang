<x-app-layout>
    ...
    <div class="py-6 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <!-- Student Info Banner -->
        <div class="bg-gradient-to-r from-slate-900 to-emerald-950 p-6 rounded-3xl text-white shadow-xl flex flex-col md:flex-row items-center justify-between gap-4 border border-emerald-500/30">
            <div class="flex items-center gap-4">
                <div class="w-14 h-14 rounded-2xl bg-emerald-600/30 border border-emerald-400/30 flex items-center justify-center font-black text-lg text-emerald-300 shadow-inner">
                    {{ strtoupper(substr($student->nama, 0, 2)) }}
                </div>
                <div>
                    <h3 class="text-lg font-black tracking-tight">{{ $student->nama }}</h3>
                    <p class="text-xs text-slate-300 font-medium">Kelas: <span class="font-bold text-emerald-300">{{ $student->kelas }}</span> | NIS: {{ $student->nis }}</p>
                </div>
            </div>
        ...
        <div class="space-y-4" x-data="{ uploadModal: null, photoModal: null }">
            ...
            @forelse($homeworks as $hw)
                ...
                <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b border-slate-100 pb-3">
                        <div>
                            <div class="flex items-center gap-2 mb-1">
                                <span class="px-2.5 py-0.5 text-[10px] font-black rounded-full bg-teal-50 text-teal-700 border border-teal-200">
                                    {{ $hw->mata_pelajaran }}
                                </span>
                    ...
                    @if($hw->lampiran_guru)
                        <div class="pt-1">
                            <a href="{{ asset('storage/' . $hw->lampiran_guru) }}" target="_blank" class="inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-50 text-emerald-700 rounded-xl text-xs font-bold border border-emerald-200 hover:bg-emerald-100 transition">
                    ...
                    <div class="bg-slate-50/80 p-4 rounded-2xl border border-slate-200/80 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                        <div>
                            <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Status Pengumpulkan Anda</div>
                            @if($sub)
                        ...
                        <div class="flex items-center gap-3 w-full md:w-auto justify-between md:justify-end">
                            @if($sub)
                                ...
                                <button @click="photoModal = '{{ asset('storage/' . $sub->foto_pr) }}'" class="px-3 py-2 bg-white text-emerald-600 hover:bg-emerald-50 rounded-xl text-xs font-bold border border-slate-200 shadow-sm transition">
                                    🖼️ Lihat Foto PR
                                </button>
                            @endif

                            <button @click="uploadModal = {{ $hw->id }}" class="px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-700 text-white hover:from-emerald-700 hover:to-teal-800 rounded-xl text-xs font-bold shadow-md shadow-emerald-600/20 transition">
                    ...
                    @if($sub && $sub->catatan_guru)
                        <div class="bg-emerald-50/70 p-4 rounded-2xl border border-emerald-100 text-xs font-medium text-emerald-900 space-y-1">
                        ...
                        <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
                            ...
                            <form action="{{ route('parent.submit-homework', $hw) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="file" name="foto_pr" required accept="image/jpeg,image/png,image/jpg,application/pdf" class="w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100">
                                </div>

                                <div>
                                    <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Catatan Tambahan Siswa (Opsional)</label>
                                    <textarea name="catatan_siswa" rows="2" placeholder="Contoh: Sudah selesai dikerjakan halaman 45 no 1-10..." class="w-full rounded-xl border-slate-200 text-xs font-medium focus:ring-emerald-500 focus:border-emerald-500"></textarea>
                                </div>

                                <div class="flex gap-2 pt-2">
                                    <button type="submit" class="flex-1 py-2.5 bg-emerald-600 text-white rounded-xl text-xs font-bold hover:bg-emerald-700 transition">
                                        Kirim Foto PR
                                    </button>
