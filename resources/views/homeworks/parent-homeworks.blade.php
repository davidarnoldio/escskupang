    ...
    <div class="py-6 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="bg-gradient-to-r from-slate-900 to-red-950 p-6 rounded-3xl text-white shadow-xl flex flex-col md:flex-row items-center justify-between gap-4 border border-red-500/30">
            ...
            <div class="flex items-center gap-3 bg-white/10 px-5 py-3 rounded-2xl border border-white/10 backdrop-blur-sm">
                ...
                <div class="text-center px-3">
                    <div class="text-[10px] font-bold text-red-300 uppercase">Sudah Kumpul</div>
                    <div class="text-lg font-black text-red-400">{{ $stats['sudah_kumpul'] }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-4" x-data="{ uploadModal: null, photoModal: null }">
            ...
            @forelse($homeworks as $hw)
                ...
                <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b border-slate-100 pb-3">
                        <div>
                            <div class="flex items-center gap-2 mb-1">
                                <span class="px-2.5 py-0.5 text-[10px] font-black rounded-full bg-teal-50 text-red-800 border border-teal-200">
                                    {{ $hw->mata_pelajaran }}
                                </span>
                    ...
                    @if($hw->lampiran_guru)
                        <div class="pt-1">
                            <a href="{{ asset('storage/' . $hw->lampiran_guru) }}" target="_blank" class="inline-flex items-center gap-2 px-3 py-1.5 bg-red-50 text-red-700 rounded-xl text-xs font-bold border border-red-200 hover:bg-red-100 transition">
                    ...
                    <div class="bg-slate-50/80 p-4 rounded-2xl border border-slate-200/80 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                        ...
                        <div class="flex items-center gap-3 w-full md:w-auto justify-between md:justify-end">
                            @if($sub)
                                ...
                                <button @click="photoModal = '{{ asset('storage/' . $sub->foto_pr) }}'" class="px-3 py-2 bg-white text-red-600 hover:bg-red-50 rounded-xl text-xs font-bold border border-slate-200 shadow-sm transition">
                                    🖼️ Lihat Foto PR
                                </button>
                            @endif
                    ...
                    @if($sub && $sub->catatan_guru)
                        <div class="bg-red-50/70 p-4 rounded-2xl border border-red-100 text-xs font-medium text-red-900 space-y-1">
                        ...
                        <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
                            ...
                            <form action="{{ route('parent.submit-homework', $hw) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="file" name="foto_pr" required accept="image/jpeg,image/png,image/jpg,application/pdf" class="w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-red-50 file:text-red-700 hover:file:bg-red-100">
                                </div>

                                <div>
                                    <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Catatan Tambahan Siswa (Opsional)</label>
                                    <textarea name="catatan_siswa" rows="2" placeholder="Contoh: Sudah selesai dikerjakan halaman 45 no 1-10..." class="w-full rounded-xl border-slate-200 text-xs font-medium focus:ring-red-500 focus:border-red-500"></textarea>
                                </div>
