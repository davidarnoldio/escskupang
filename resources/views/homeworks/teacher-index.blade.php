        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4" x-data="{ targetType: 'all' }">
                <h3 class="text-base font-bold text-slate-800 flex items-center gap-2">
                    <svg class="w-5 h-5 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                ...
                <form action="{{ route('homeworks.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div>
                        ...
                        @else
                            <select name="kelas" required class="w-full rounded-xl border-slate-200 text-xs font-medium focus:ring-red-500 focus:border-red-500">
                    ...
                    <div>
                        ...
                        <input type="text" name="mata_pelajaran" required placeholder="Contoh: Matematika / Bahasa Indonesia" class="w-full rounded-xl border-slate-200 text-xs font-medium focus:ring-red-500 focus:border-red-500">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Judul PR / Tugas</label>
                        <input type="text" name="judul" required placeholder="Contoh: Latihan Perkalian Bab 3 Halaman 45" class="w-full rounded-xl border-slate-200 text-xs font-medium focus:ring-red-500 focus:border-red-500">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Batas Waktu Pengumpulan (Deadline)</label>
                        <input type="datetime-local" name="deadline" required value="{{ date('Y-m-d\TH:i', strtotime('+2 days 23:59')) }}" class="w-full rounded-xl border-slate-200 text-xs font-medium focus:ring-red-500 focus:border-red-500">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Deskripsi PR & Instruksi Pengerjaan</label>
                        <textarea name="deskripsi" rows="3" required placeholder="Kerjakan soal no 1-10 di buku tulis, lalu foto hasil pengerjaan secara jelas..." class="w-full rounded-xl border-slate-200 text-xs font-medium focus:ring-red-500 focus:border-red-500"></textarea>
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase mb-1">File Lampiran Guru (Opsional)</label>
                        <input type="file" name="lampiran_guru" class="w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-red-50 file:text-red-700 hover:file:bg-red-100">
                    </div>
                        ...
                        <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                            <div class="space-y-1.5 flex-1">
                                ...
                                <div class="flex items-center gap-4 text-[11px] text-slate-400 font-semibold pt-1">
                                    ...
                                    <span>📬 Pengumpulan: <strong class="text-red-600">{{ $hw->submissions->count() }} Siswa</strong></span>
                                </div>
                            </div>
                            ...
                            <div class="shrink-0 flex items-center gap-2 w-full md:w-auto">
                                <a href="{{ route('homeworks.submissions', $hw) }}" class="flex-1 md:flex-initial px-4 py-2.5 bg-gradient-to-r from-red-600 to-red-800 text-white hover:from-red-700 hover:to-red-900 rounded-xl text-xs font-bold shadow-md shadow-red-500/20 transition flex items-center justify-center gap-2">
