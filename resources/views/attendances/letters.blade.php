                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse($attendances as $att)
                                <tr class="hover:bg-slate-50 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        @if($att->status == 'sakit')
                                            <span class="px-2.5 py-0.5 rounded-md text-xs font-extrabold bg-red-100 text-red-900 border border-red-300 uppercase">
                                                🏥 Sakit
                                            </span>
                                        @else
        ...
        <div x-show="showModal" class="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-xs flex items-center justify-center p-4" x-cloak>
            <div class="bg-white rounded-3xl max-w-2xl w-full p-6 shadow-2xl space-y-4">
                ...
                <div class="flex justify-between items-center pt-2">
                    <a :href="modalImg" target="_blank" class="px-4 py-2 bg-red-50 hover:bg-red-100 text-red-700 font-bold text-xs rounded-xl transition inline-flex items-center gap-1">
                        🔗 Buka File Asli
                    </a>
                    <button type="button" @click="showModal = false" class="px-5 py-2 bg-slate-900 hover:bg-slate-800 text-red-400 font-extrabold text-xs rounded-xl transition">
                        Tutup
                    </button>
                </div>
            </div>
