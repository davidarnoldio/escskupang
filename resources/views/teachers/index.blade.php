<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div>
                <a href="{{ route('teachers.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs rounded-2xl shadow-lg shadow-blue-600/30 transition duration-200 cursor-pointer">
    ...
    <div class="space-y-6">
        ...
        <!-- Search Filter Bar -->
        <div class="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
            <form method="GET" action="{{ route('teachers.index') }}" class="w-full md:w-auto flex items-center gap-2">
                <div class="relative w-full md:w-80">
                    <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari nama atau email guru..." class="w-full pl-9 pr-3.5 py-2.5 text-xs font-semibold rounded-2xl bg-slate-50 border-slate-200 focus:ring-2 focus:ring-blue-600 focus:bg-white transition">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                    </div>
                </div>
                <button type="submit" class="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-extrabold text-xs rounded-2xl shadow-sm transition cursor-pointer">Cari</button>
                @if(!empty($search))
                    <a href="{{ route('teachers.index') }}" class="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-extrabold text-xs rounded-2xl transition">Reset</a>
                @endif
            </form>
            <div class="text-xs font-extrabold text-slate-500">
                Total Guru: <span class="text-blue-600 font-black text-sm">{{ $teachers->count() }}</span> Akun Terdaftar
            </div>
        </div>
            ...
            @foreach(array_merge($officialClasses, ['Unassigned']) as $class)
                ...
                <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                    ...
                    <div class="px-6 py-4 bg-slate-50/60 border-b border-slate-100 flex items-center justify-between">
                        ...
                        <span class="px-3 py-1 bg-white text-slate-700 font-extrabold text-xs rounded-full border border-slate-200 shadow-2xs">
                        ...
                        @else
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                                @foreach($classTeachers as $teacher)
                                    <div class="bg-slate-50/80 rounded-2xl p-5 border border-slate-200/80 space-y-4 hover:border-blue-300 hover:bg-blue-50/30 transition duration-200 relative flex flex-col justify-between">
                                        <div>
                                            <div class="flex items-start justify-between gap-3">
                                                <div class="flex items-center gap-3">
                                                    <div class="w-10 h-10 rounded-full bg-blue-600 text-white font-black text-xs flex items-center justify-center shadow-xs">
                                                        {{ strtoupper(substr($teacher->name, 0, 2)) }}
                                                    </div>
                                                    <div>
                                                        <h4 class="font-extrabold text-slate-900 text-xs leading-tight">{{ $teacher->name }}</h4>
                                                        <p class="text-[11px] text-slate-500 font-mono mt-0.5">{{ $teacher->email }}</p>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="mt-4 pt-3 border-t border-slate-200/60 space-y-2">
                                                <div class="flex items-center justify-between text-[11px]">
                                                    ...
                                                    <span class="font-extrabold text-blue-700 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-200">
