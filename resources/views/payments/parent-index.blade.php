<x-app-layout>
    ...
    <div class="py-6 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="bg-gradient-to-r from-slate-900 to-emerald-950 p-6 rounded-3xl text-white shadow-xl flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    ...
                    <div class="shrink-0 flex items-center gap-2 w-full md:w-auto">
                        @if($p->bukti_pembayaran)
                            <button @click="proofModal = '{{ asset('storage/' . $p->bukti_pembayaran) }}'" class="flex-1 md:flex-initial px-3 py-2 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 rounded-xl text-xs font-bold border border-emerald-200 transition">
                                🔍 Lihat Bukti Upload
                            </button>
                        @endif

                        @if(in_array($p->status, ['belum_lunas', 'ditolak']))
                            <button @click="uploadModal = {{ $p->id }}" class="flex-1 md:flex-initial px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-700 text-white hover:from-emerald-700 hover:to-teal-800 rounded-xl text-xs font-bold shadow-md shadow-emerald-600/20 transition">
                        ...
                        <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
                            ...
                            <form action="{{ route('parent.upload-proof', $p) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="file" name="bukti_pembayaran" required accept="image/jpeg,image/png,image/jpg,application/pdf" class="w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100">
                                </div>
