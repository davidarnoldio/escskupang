        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-xl lg:text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
                    <span>Data Siswa ESCS Kupang</span>
                </h2>
    ...
    <div class="space-y-6" x-data="{ qrModalOpen: false, selectedStudent: null, openQrModal(student) { this.selectedStudent = student; this.qrModalOpen = true; } }">
        ...
        <div class="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 space-y-3">
            <div class="flex items-center justify-between">
                ...
                @if($assignedClass)
                    <span class="px-3 py-1 bg-blue-50 text-blue-800 border border-blue-200 font-extrabold text-xs rounded-full">
            ...
            <div class="flex items-center gap-2 overflow-x-auto pb-1">
                @if(!$assignedClass)
                    <a href="{{ route('students.index', ['search' => request('search')]) }}" 
                       class="px-4 py-2 rounded-2xl text-xs font-bold transition duration-200 shrink-0 {{ !request('kelas') ? 'bg-blue-600 text-white font-black shadow-md shadow-blue-600/30' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                        Semua Kelas
                    </a>
                @endif
                @foreach($classList as $c)
                    @if(!$assignedClass || $assignedClass == $c)
                        <a href="{{ route('students.index', ['kelas' => $c, 'search' => request('search')]) }}" 
                           class="px-4 py-2 rounded-2xl text-xs font-bold transition duration-200 shrink-0 {{ request('kelas') == $c || $assignedClass == $c ? 'bg-blue-600 text-white font-black shadow-md shadow-blue-600/30' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                            {{ $c }}
                        </a>
                    @endif
                @endforeach
            ...
            <form method="GET" action="{{ route('students.index') }}" class="flex flex-col sm:flex-row gap-3">
                <div class="flex-1 relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama siswa atau NIS..."
                           class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-semibold focus:ring-2 focus:ring-blue-600 focus:bg-white transition">
                </div>
        ...
        <!-- Students Data Table Container matching Reference Image 2 -->
                            ...
                            <tr class="hover:bg-blue-50/40 transition duration-150">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        @else
                                            <div class="w-9 h-9 rounded-full bg-blue-600 text-white flex items-center justify-center font-black text-xs shadow-2xs">
                ...
                <template x-if="selectedStudent">
                    <div>
                        ...
                        <div class="flex items-center justify-center gap-3">
                            <a :href="'/students/' + selectedStudent.id + '/qr-card'" target="_blank" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs rounded-xl transition shadow-md">
                                Cetak Kartu QR
                            </a>
