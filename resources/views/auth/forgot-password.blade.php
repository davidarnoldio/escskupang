<x-guest-layout>
    <div class="mb-6 text-center">
        ...
        <div class="inline-flex items-center justify-center mb-3 transform hover:scale-105 transition-all duration-300">
            <img src="{{ asset('images/logo.png') }}" alt="Logo ESCS Kupang" class="h-20 w-auto object-contain drop-shadow-xl" onerror="this.src='{{ asset('logo.png') }}'">
        </div>
        <h2 class="text-2xl font-bold text-slate-900 tracking-tight">Lupa Kata Sandi Akun</h2>
        <p class="text-xs font-semibold text-sky-600 mt-1">Excellent Spirit Christian School Kupang</p>
    </div>

    <div class="mb-5 p-4 bg-sky-50/80 border border-sky-100 rounded-2xl text-xs text-sky-900 leading-relaxed font-medium">
    ...
    <form method="POST" action="{{ route('password.email') }}" class="space-y-4">
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus placeholder="nama@escs-kupang.sch.id atau nama@student.sch.id"
                       class="w-full pl-9 pr-3 py-2.5 bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-xl focus:ring-2 focus:ring-sky-500 focus:border-sky-500 focus:bg-white transition duration-200">
            </div>
